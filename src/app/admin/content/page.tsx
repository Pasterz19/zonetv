...
async function updateMovie(formData: FormData) {
  ...
  await runQuery(
    `UPDATE Movie SET title = ?, category = ?, description = ?, imageUrl = ?, externalUrl = ?, duration = ?, releaseYear = ?, updatedAt = datetime('now') WHERE id = ?`,
    [title, category, description, imageUrl, externalUrl, duration, releaseYear, id]
  );
...
async function updateSeries(formData: FormData) {
  ...
  await runQuery(
    `UPDATE Series SET title = ?, category = ?, description = ?, imageUrl = ?, releaseYear = ?, updatedAt = datetime('now') WHERE id = ?`,
    [title, category, description, imageUrl, releaseYear, id]
  );
...
async function updateChannel(formData: FormData) {
  ...
  await runQuery(
    `UPDATE Channel SET name = ?, imageUrl = ?, streamUrl = ?, groupTitle = ?, enabled = ?, updatedAt = datetime('now') WHERE id = ?`,
    [name, imageUrl, streamUrl, groupTitle || null, enabled, id]
  );
...
export { createMovie, deleteMovie, deleteManyMovies, createSeries, deleteSeries, deleteManySeries, createChannel, deleteChannel, deleteManyChannels, updateMovie, updateSeries, updateChannel };
...
export default async function ContentManagementPage() {
  ...
  const [moviesRaw, seriesRaw, channelsRaw] = await Promise.all([
    query<{
      ...
      createdAt: string;
    }>('SELECT id, title, category, description, imageUrl, externalUrl, duration, releaseYear, createdAt FROM Movie ORDER BY createdAt DESC'),
    query<{
      id: string;
      title: string;
      category: string;
      ...
      `SELECT s.id, s.title, s.category, s.description, s.imageUrl, s.releaseYear, s.createdAt,
